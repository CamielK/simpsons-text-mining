...
def load_csv(filename, startOffset, numRecords):
    ...
    with codecs.open(filerealname, encoding="utf-8") as csvfile:
        ...
        for row in readCSV:
            ...
            try:
                ...
                if (numRecords == 0 or (i >= startOffset and i < startOffset + numRecords)): data.append(row)
                if (numRecords > 0 and i > numRecords): break
            except:
                print('read error on line: ' + str(i))
    return data
